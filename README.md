...
* 顯示卡 (GPU)：必須配備 NVIDIA 獨立顯示卡（建議 RTX 4060 或同等類型以上），且系統需安裝支援 CUDA 之外顯驅動程式，以召喚 FFmpeg 底層之 `h264_nvenc` 硬體編碼加速引擎[cite: 8]。
...
* 處理器 (CPU)：具備多核心之處理器（系統預設調度 8 核心進行非同步加速）[cite: 14]。
